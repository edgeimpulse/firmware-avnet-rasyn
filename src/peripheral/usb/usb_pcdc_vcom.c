...
static uint8_t g_comms_opened_flag = 0;

fsp_err_t comms_send_low(uint8_t * p_src, uint32_t len, uint32_t period);

void usb_composite_callback(usb_event_info_t * event, usb_hdl_t handle, usb_onoff_t onoff)
{
...
fsp_err_t comms_suspend(void)
{
    ...
    err = g_usb_on_usb.suspend(&g_basic_ctrl);
    if (FSP_SUCCESS != err)
    {
        return (err);
    }
...
fsp_err_t comms_resume(void)
{
    ...
    err = g_usb_on_usb.resume(&g_basic_ctrl);
    if (FSP_SUCCESS != err)
    {
        return (err);
    }
